...
<<<<<<< HEAD
# 1 Import data
Our first step is to import packages and data into R. The trait data and OMI 
data also need to be merged.
=======
...
>>>>>>> 843e2ba0c557428b2b5b436abcc9bb73f3de5de9
...
dataset <- subset(dataset, select = -c(AVGBodyMass, CarryWeight, PullStrength))
```
...
```
#Precipitation
Multiple variables are tightly correlated because they are aspects of 
precipitation, we are going to remove them 1 by 1 in order of decreasing VIF.
We start with bio14 (precipitation of the driest month) because it has the 
highest VIF value. 
```{r variable elimination, results='markup'}
startPredictors$bio14 <- NULL
vif(startPredictors)

startPredictors$bio18 <- NULL
vif(startPredictors)

startPredictors$bio15 <- NULL
vif(startPredictors)
...
#Temperature
Multiple variables are tightly correlated because they are aspects of 
temperature, the next step is to remove these variables 1 by 1 in order of 
decreasing VIF
```{}
...
#collinearity using the visualized matrix is done. During the first round, 
#AdultBodyMass was removed due to a high collinearity with other traits. 
#However, because Adultbodymass is hypothesized to be a biologically relevant 
#trait, it will be reinserted in this step.
...
# Construct model
phyloGLM <- phyloglm(formula = finalFormula, data = dataset, phy = tree, 

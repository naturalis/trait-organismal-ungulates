...
Inspired by: https://stats.stackexchange.com/questions/547024/what-are-pitfalls-of-clustering-variables-by-absolute-correlation-for-variable-s?newreg=23f15a0b98b34912a8ffd0d7450d937a

Unless a statistician steps in (have you asked?) a reasonable procedure as far
as I can see is the following:

1. For each column individually, do the phyloglm (i.e. Domestication ~ column)
   and record the AIC of the model.
2. Calculate the correlation matrix (as below).
3. Perform complete agglomerative clustering to identify the islands in the 
   correlation matrix.
4. For each cluster, iteratively remove variables in ascending order of AIC, 
   i.e. poor fitting variables are thrown out first. Calculate the VIF after
   each removal, and stop until VIF<10 for all remaining members of the cluster.
5. Do this for all clusters, then move on to model selection.
...
corrplot(matrix, type="full", order = "hclust", tl.pos = "lt", tl.col = "black", tl.cex = 0.4, diag=F)

...
with finding out which traits have a very high collinearity, so they can be removed and a VIF analysis 
...
correlation matrix. With the findCorrelation function, the columns with the highest collinearity is 
...
plotMatrix <- cor(plotPredictors, use = "pairwise.complete.obs")
...
findCorrelation(plotMatrix, cutoff = .9, verbose = FALSE, exact=FALSE)
...
setPredictors <- columns[,c(1,5:7,10,12,14,18,22:25,27,29:31,33,50,52)]
...
# important for this research, so X9.1_GestationLen_d is removed.
...
```{r VIF}
# (without X5.1_AdultBodyMass_g)
# Round1 - Results: remove X9.1_GestationLen_d
vif(setPredictors)

# Round2 - Results: remove AVGFoodConsumption
setPredictors <- setPredictors[,c(1,2,4:19)]
vif(setPredictors)

# Round3 
setPredictors <- setPredictors[,c(1:6,8:18)]
vif(setPredictors)


# (with X5.1_AdultBodyMass_g - we wanna hold so is considered as below 10)
...
## (without X5.1_AdultBodyMass_g)
# X1.1_ActivityCycle, X6.1_DietBreadth, X12.1_HabitatBreadth, X15.1_LitterSize, 
# X21.1_PopulationDensity_n_km2, X10.2_SocialGrpSize, MatingSystem, YearRoundBreeding, 
# DevelopmentStrategy, Horns_Antlers, NaturalPredators, AVGMovingSpeed, AVGTravelDistance, Aspect,
# ClayPercentage, OrganicCarbon, Slope
predictors <- columns[,c(1,5,7,10,12,14,22:25,27,29:31,33,50,52)]

## (with X5.1_AdultBodyMass_g)

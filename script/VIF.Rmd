...
A correlation matrix can help with finding out which traits have a very high collinearity
...
test <- columns[,c(1,2,5,7,8,10,12,14,18,19,22:27,29,31,33,42,50,70,71)]
plotMatrix <- cor(test, use = "pairwise.complete.obs")
rownames(plotMatrix) <- c(1,2,5,7,8,10,12,14,18,19,22:27,29,31,33,42,50,70,71)
colnames(plotMatrix) <- c(1,2,5,7,8,10,12,14,18,19,22:27,29,31,33,42,50,70,71)
corrplot(plotMatrix, type = "upper", order = "hclust", tl.pos = 'd', tl.col = 'black', tl.cex = 0.6)
...
Some traits have a very high collinearity with other traits, because they fall into the same category 
(for example LitterSize and TeatNumber, which both have to do with offspring). When there is a very high collinearity, the VIF analysis shows only infinite instead of numbers as a VIF value. Because of this, the removal of very high collinearity needs to be done in smaller steps.
...
# (those two traits have a high collinearity, because CarryWeight was calculated using the bodymass) the 
...
When the trait and OMI data is merged and analysed, a high collinearity is seen. To keep it organized,
